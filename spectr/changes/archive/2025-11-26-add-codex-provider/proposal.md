...
Codex CLI is OpenAI's open-source CLI for agentic coding. It uses custom prompts
(slash commands) stored in `~/.codex/prompts/` with YAML frontmatter. Adding
Codex as a provider expands Spectr's reach to users of this emerging AI coding
tool.
...
- **BREAKING PATTERN**: First provider using global paths rather than
  project-level
...
- Affected code: `internal/init/providers/provider.go` (may need global path
  support)
