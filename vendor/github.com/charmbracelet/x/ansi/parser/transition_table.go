...
func (t TransitionTable) SetDefault(
	action Action,
	state State,
) {
	for i := range t {
...
func (t TransitionTable) AddMany(
	codes []byte,
	state State,
	action Action,
	next State,
...
func (t TransitionTable) AddRange(
	start, end byte,
	state State,
	action Action,
	next State,
...
func (t TransitionTable) Transition(
	state State,
	code byte,
) (State, Action) {
	index := int(
...
func GenerateTransitionTable() TransitionTable {
	...
	table.AddRange(
		...
		PutAction,
		DcsStringState,
	) // Allow Utf8 characters by extending the printable range from 0x7F to 0xFF
