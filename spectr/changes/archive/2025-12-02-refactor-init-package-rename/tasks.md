...
- [x] **Update package declarations**: Change `package init` to `package initialize` in all `.go` files in `internal/initialize/`
- [x] **Update internal self-references**: Update import path in `executor.go` and `wizard.go` for providers subpackage
- [x] **Update cmd/init.go imports**: Change import paths and remove unnecessary alias
