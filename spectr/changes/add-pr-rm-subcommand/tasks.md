...
- [x] 1.3 Register `rm` subcommand in `PRCmd` struct with aliases `r` and `remove`
- [x] 1.4 Implement `Run()` method for `PRRemoveCmd`

## 2. Workflow Support

- [x] 2.1 Add remove branch prefix handling in `prepareWorkflowContext()` (`spectr/remove/<change-id>`)
- [x] 2.2 Create `removeChangeInWorktree()` function in `internal/pr/helpers.go`
- [x] 2.3 Add remove case to `executeOperation()` switch statement in `workflow.go`
...
- [x] 4.1 Add remove mode handling to `executeDryRun()` in `internal/pr/dryrun.go`
...
- [x] 5.3 Add unit tests for `removeChangeInWorktree()` in `internal/pr/helpers_test.go`
- [x] 5.4 Add integration test for remove workflow in `internal/pr/integration_test.go`
