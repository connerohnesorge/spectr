...
- [ ] Define `BaseSlashCommandFrontmatter` map with frontmatter for proposal
  and apply commands
- [ ] Implement `GetBaseFrontmatter(cmd domain.SlashCommand)` method with deep
  copy
- [ ] Implement deep copy helper `copyMap(src map[string]interface{})` for
  copying frontmatter
...
- [ ] Implement `ApplyFrontmatterOverrides(base, overrides)` in `frontmatter.go`
  - Copy base map
  - Apply Set operations (merge/overwrite)
  - Apply Remove operations (delete keys)
  - Handle nil overrides (return copy of base)
- [ ] Implement `RenderFrontmatter` using `gopkg.in/yaml.v3`
  - Signature: `RenderFrontmatter(fm, body string) (string, error)`
  - Marshal fm to YAML
  - Wrap in `---` delimiters
  - Append body content
...
- [ ] Edit `slash-proposal.md.tmpl`: remove `---` frontmatter block, keep only body
- [ ] Edit `slash-apply.md.tmpl`: remove `---` frontmatter block, keep only body
- [ ] Update any tests that depend on template content format

## Phase 5: TemplateManager Extension

- [ ] Add method to `TemplateManager` in `internal/initialize/templates.go`:
  Signature: `SlashCommandWithOverrides(cmd, overrides)`
...
- [ ] Modify `ClaudeProvider.Initializers()` in `providers/claude.go`:
  - For proposal, use:

    ```go
    tm.SlashCommandWithOverrides(domain.SlashProposal,
      &domain.FrontmatterOverride{Set: map[string]interface{}{"context": "fork"},
                                   Remove: []string{"agent"}})
    ```
