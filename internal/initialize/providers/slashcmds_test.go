...
func TestSlashCommandsInitializer_Init(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantCreated:  []string{".claude/commands/spectr/apply.md"},
			wantUpdated:  []string{".claude/commands/spectr/proposal.md"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create initializer
			init := NewSlashCommandsInitializer(tt.dir, commands)
			...
			// Check result
			if !stringSliceEqualUnordered(result.CreatedFiles, tt.wantCreated) {
				t.Errorf("CreatedFiles = %v, want %v", result.CreatedFiles, tt.wantCreated)
			}
			if !stringSliceEqualUnordered(result.UpdatedFiles, tt.wantUpdated) {
				t.Errorf("UpdatedFiles = %v, want %v", result.UpdatedFiles, tt.wantUpdated)
			}

			// Verify files exist and have correct content
			proposalContent, err := afero.ReadFile(projectFs, filepath.Join(tt.dir, "proposal.md"))
			if err != nil {
			...
			applyContent, err := afero.ReadFile(projectFs, filepath.Join(tt.dir, "apply.md"))
			if err != nil {
...
func TestSlashCommandsInitializer_IsSetup(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			dir:           ".claude/commands/spectr",
			existingFiles: []string{".claude/commands/spectr/proposal.md"},
			want:          false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create templates
			commands := map[domain.SlashCommand]domain.TemplateRef{
				domain.SlashProposal: createTestTemplate(t, "proposal"),
				domain.SlashApply:    createTestTemplate(t, "apply"),
			}
...
func TestSlashCommandsInitializer_dedupeKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			init := &SlashCommandsInitializer{dir: tt.dir, commands: commands}
			got := init.dedupeKey()
			if got != tt.want {
...
func TestHomeSlashCommandsInitializer_Init(t *testing.T) {
	...
	// Create initializer
	init := NewHomeSlashCommandsInitializer(dir, commands)
	...
	if len(result.CreatedFiles) != 2 {
		t.Errorf("CreatedFiles = %v, want 2 files", result.CreatedFiles)
	}
	...
	if !exists {
		t.Error("proposal.md should exist in home filesystem")
	}

	exists, _ = afero.Exists(homeFs, applyPath)
	if !exists {
		t.Error("apply.md should exist in home filesystem")
	}

	// Verify files DO NOT exist in project filesystem
	exists, _ = afero.Exists(projectFs, proposalPath)
	if exists {
		t.Error("proposal.md should NOT exist in project filesystem")
	}
}
...
func TestHomeSlashCommandsInitializer_dedupeKey(t *testing.T) {
	...
	init := &HomeSlashCommandsInitializer{dir: ".config/mytool", commands: commands}
	got := init.dedupeKey()
...
func TestPrefixedSlashCommandsInitializer_Init(t *testing.T) {
	...
	// Create initializer
	init := NewPrefixedSlashCommandsInitializer(dir, prefix, commands)
	...
	proposalPath := filepath.Join(dir, "spectr-proposal.md")
	applyPath := filepath.Join(dir, "spectr-apply.md")

	proposalContent, err := afero.ReadFile(projectFs, proposalPath)
	if err != nil {
		t.Fatalf("failed to read spectr-proposal.md: %v", err)
	}
	if string(proposalContent) != "proposal content" {
		t.Errorf(
	...
	applyContent, err := afero.ReadFile(projectFs, applyPath)
	if err != nil {
		t.Fatalf("failed to read spectr-apply.md: %v", err)
	}
	if string(applyContent) != "apply content" {
		t.Errorf("spectr-apply.md content = %q, want %q", string(applyContent), "apply content")
	}
}
...
func TestPrefixedSlashCommandsInitializer_IsSetup(t *testing.T) {
	...
	if init.IsSetup(projectFs, homeFs, cfg) {
		t.Error("IsSetup() = true, want false when files don't exist")
	}
	...
	if !init.IsSetup(projectFs, homeFs, cfg) {
		t.Error("IsSetup() = false, want true when files exist")
	}
}
...
func TestHomePrefixedSlashCommandsInitializer_Init(t *testing.T) {
	...
	// Create initializer
	init := NewHomePrefixedSlashCommandsInitializer(dir, prefix, commands)
	...
	proposalContent, err := afero.ReadFile(homeFs, proposalPath)
	if err != nil {
		t.Fatalf("failed to read spectr-proposal.md from home fs: %v", err)
	}
	if string(proposalContent) != "proposal content" {
		t.Errorf(
			"spectr-proposal.md content = %q, want %q",
	...
	applyContent, err := afero.ReadFile(homeFs, applyPath)
	if err != nil {
		t.Fatalf("failed to read spectr-apply.md from home fs: %v", err)
	}
	if string(applyContent) != "apply content" {
		t.Errorf("spectr-apply.md content = %q, want %q", string(applyContent), "apply content")
	}
	...
	exists, _ := afero.Exists(projectFs, proposalPath)
	if exists {
		t.Error("spectr-proposal.md should NOT exist in project filesystem")
	}
}
...
func TestTOMLSlashCommandsInitializer_Init(t *testing.T) {
	...
	// Create templates
	commands := map[domain.SlashCommand]domain.TemplateRef{
		domain.SlashProposal: createTestTemplate(t, "proposal toml content"),
		domain.SlashApply:    createTestTemplate(t, "apply toml content"),
	}

	// Create initializer
	init := NewTOMLSlashCommandsInitializer(dir, commands)
	...
	proposalPath := filepath.Join(dir, "proposal.toml")
	...
	if string(proposalContent) != "proposal toml content" {
		t.Errorf(
	...
	applyContent, err := afero.ReadFile(projectFs, applyPath)
	if err != nil {
		t.Fatalf("failed to read apply.toml: %v", err)
	}
	if string(applyContent) != "apply toml content" {
		t.Errorf("apply.toml content = %q, want %q", string(applyContent), "apply toml content")
	}
}
...
func TestTOMLSlashCommandsInitializer_dedupeKey(t *testing.T) {
	...
	init := &TOMLSlashCommandsInitializer{dir: ".gemini/commands/spectr", commands: commands}
	got := init.dedupeKey()
...
func TestSlashCommands_TemplateContextUsage(t *testing.T) {
	// Setup
	projectFs := afero.NewMemMapFs()
	homeFs := afero.NewMemMapFs()
	...
	commands := map[domain.SlashCommand]domain.TemplateRef{
		domain.SlashProposal: domain.TemplateRef{Name: "test", Template: tmpl},
	}

	// Create initializer
	init := NewSlashCommandsInitializer(dir, commands)
	...
	content, err := afero.ReadFile(projectFs, filepath.Join(dir, "proposal.md"))
	if err != nil {
...
func TestSlashCommands_DifferentTypesHaveDifferentDedupeKeys(t *testing.T) {
	// Verify that different initializer types with same directory have different dedupe keys
	commands := map[domain.SlashCommand]domain.TemplateRef{
		domain.SlashProposal: createTestTemplate(t, "content"),
	}

	dir := ".claude/commands/spectr"

	slashInit := &SlashCommandsInitializer{dir: dir, commands: commands}
	homeSlashInit := &HomeSlashCommandsInitializer{dir: dir, commands: commands}
	tomlInit := &TOMLSlashCommandsInitializer{dir: dir, commands: commands}
	...
	for i, key := range keys {
		if seen[key] {
			t.Errorf("duplicate dedupe key at index %d: %s", i, key)
		}
		seen[key] = true
	}
