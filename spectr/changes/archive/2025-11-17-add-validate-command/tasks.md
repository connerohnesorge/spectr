...
- [x] 1.2.2 Implement `ExtractSections(content string) map[string]string` for
  `##` headers

- [x] 1.2.3 Implement `ExtractRequirements(content string) []Requirement` for
  `###` headers

- [x] 1.2.4 Implement `ExtractScenarios(requirementBlock string) []string` for
  `####` headers
- [x] 1.2.5 Implement `ContainsShallOrMust(text string) bool` regex checker
- [x] 1.2.6 Implement `NormalizeRequirementName(name string) string` for
  duplicate detection
...
- [x] 1.3.4 Handle missing directories gracefully (return empty slice, not
  error)
...
- [x] 2.1.2 Implement `ValidateSpecFile(path string, strictMode bool)
...
- [x] 2.1.8 Check each requirement has at least one scenario (WARNING if
  missing)
...
- [x] 2.2.2 Implement `ValidateChangeDeltaSpecs(changeDir string, strictMode
  bool) (*ValidationReport, error)`
- [x] 2.2.3 Scan changeDir/specs/ for capability subdirectories
- [x] 2.2.4 For each spec.md, parse delta sections (##
...
- [x] 2.2.8 For ADDED requirements: check SHALL/MUST and scenarios (ERROR if
  missing)
- [x] 2.2.9 For MODIFIED requirements: check SHALL/MUST and scenarios (ERROR if
  missing)
...
- [x] 2.3.4 Implement `ValidateSpec(path string) (*ValidationReport, error)`
  wrapper
- [x] 2.3.5 Implement `ValidateChange(changeDir string) (*ValidationReport,
  error)` wrapper
- [x] 2.3.6 Add helper method `CreateReport(issues []ValidationIssue)
  *ValidationReport`
...
- [x] 3.1.10 Add `NoInteractive bool` field with `name:"no-interactive"
  help:"..."`
...
- [x] 3.2.4 If no ItemName and interactive mode, call `runInteractiveMode()` and
  return
...
- [x] 3.5.4 Show options: "All", "All changes", "All specs", "Pick specific
  item"
- [x] 3.5.5 If "Pick specific item", show second prompt with all changes and
  specs
...
- [x] 3.6.1 Create `printReport(itemType, itemName string, report
  *ValidationReport)` method
...
- [x] 4.1.3 Add appropriate struct tag: `cmd:"" help:"Validate specs and
  changes"`
