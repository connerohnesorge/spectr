...
func TestGetChangeItems(t *testing.T) {
	...
	for _, item := range items {
		...
		expectedPath := filepath.Join(tmpDir, config.DefaultRootDir, "changes", item.Name)
...
func TestGetChangeItems_PathConstruction(t *testing.T) {
	...
	expectedPath := filepath.Join(tmpDir, config.DefaultRootDir, "changes", "add-feature")
...
func TestGetSpecItems(t *testing.T) {
	...
	for _, item := range items {
		...
		expectedPath := filepath.Join(tmpDir, config.DefaultRootDir, "specs", item.Name, "spec.md")
		assert.Equal(t, expectedPath, item.Path)
	}
}
...
func TestGetSpecItems_PathConstruction(t *testing.T) {
	...
	expectedPath := filepath.Join(tmpDir, config.DefaultRootDir, "specs", "user-auth", "spec.md")
	assert.Equal(t, expectedPath, items[0].Path)
}
...
func TestGetChangeItems_WithArchive(t *testing.T) {
	...
	archiveDir := filepath.Join(tmpDir, config.DefaultRootDir, "changes", "archive", "old-change")
...
func TestGetChangeItems_WithHidden(t *testing.T) {
	...
	hiddenDir := filepath.Join(tmpDir, config.DefaultRootDir, "changes", ".hidden")
...
func TestGetSpecItems_WithHidden(t *testing.T) {
	...
	// Create hidden directory
	hiddenDir := filepath.Join(tmpDir, config.DefaultRootDir, "specs", ".hidden")
	err := os.MkdirAll(hiddenDir, testDirPerm)
	assert.NoError(t, err)
