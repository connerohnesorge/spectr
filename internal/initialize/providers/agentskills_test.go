...
func (*mockSkillTemplateManager) SlashCommand(
...
func (*mockSkillTemplateManager) TOMLSlashCommand(
	cmd domain.SlashCommand,
) domain.TemplateRef {
	return domain.TemplateRef{
		Name: fmt.Sprintf(
...
func (m *mockSkillTemplateManager) SkillFS(
	_ string,
) (fs.FS, error) {
	if m.err != nil {
		return nil, m.err
...
func TestAgentSkillsInitializer_Construction(
	...
	if init == nil {
		t.Fatal(
			"NewAgentSkillsInitializer returned nil",
		)
	}
...
func TestAgentSkillsInitializer_Init_CopySkillDirectory(
	t *testing.T,
) {
	...
	for _, tt := range tests {
		...
		if !exists {
			t.Errorf(
				"file %s does not exist after Init()",
				tt.path,
			)
...
func TestAgentSkillsInitializer_Init_SkillNotFound(
	t *testing.T,
) {
	tm := &mockSkillTemplateManager{
		err: fs.ErrNotExist,
	...
	if err == nil {
		t.Fatal(
			"Init() should fail when skill not found",
		)
	}
...
func TestAgentSkillsInitializer_Init_IdempotentExecution(
	t *testing.T,
) {
	// Setup: Create a mock skill filesystem
	skillFS := fstest.MapFS{
	...
	if len(result1.CreatedFiles) == 0 {
		t.Error(
			"First Init() should create files",
		)
	}
	if len(result1.UpdatedFiles) != 0 {
		t.Error(
			"First Init() should not update files",
		)
	}
...
func TestAgentSkillsInitializer_Init_CreatesTargetDirectory(
	t *testing.T,
) {
	skillFS := fstest.MapFS{
		"SKILL.md": {
	...
	if exists {
		t.Fatal(
			"target directory should not exist before Init()",
		)
	}
	...
	if err != nil {
		t.Errorf(
			"failed to check directory: %v",
			err,
		)
	}
	if !exists {
		t.Error(
			"target directory should exist after Init()",
		)
	}
	...
	if err != nil {
		t.Errorf(
			"failed to check parent directory: %v",
			err,
		)
	}
	if !exists {
		t.Error(
			"parent directory should exist after Init()",
		)
	}
}
...
func TestAgentSkillsInitializer_IsSetup(
	t *testing.T,
) {
	tests := []struct {
		name       string
...
func TestAgentSkillsInitializer_dedupeKey(
	t *testing.T,
) {
	tests := []struct {
		name      string
...
func TestAgentSkillsInitializer_PreservesExecutablePermissions(
	t *testing.T,
) {
	...
	tests := []struct {
		...
		{
			".claude/skills/test-skill/scripts/executable.sh",
			0o755,
		},
		{
			".claude/skills/test-skill/scripts/readonly.sh",
			0o755,
		}, // All .sh files are executable
		{
			".claude/skills/test-skill/config/settings.json",
			0o644,
		},
	}
...
func TestAgentSkillsInitializer_EmptySkillDirectory(
	t *testing.T,
) {
...
func containsAll(
	actualList, expectedList []string,
) bool {
