...
- **AND** no error occurs
- **AND** the file is marked as "updated" rather than "created" in execution
  result
...
- **THEN** they find the tool mapping integrated into the tool registry
  configuration
...
- **WHEN** a developer reviews the ToolDefinition struct in
  `internal/init/models.go`
- **THEN** the struct contains: ID (type-safe ToolID), Name, Type, Priority, and
  Configured fields
- **AND** the struct does NOT contain a ConfigPath field
- **AND** file paths are determined by configurator implementations, not the
  registry
