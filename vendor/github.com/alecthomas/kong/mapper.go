...
var (
	mapperValueType = reflect.TypeOf((*MapperValue)(nil)).
			Elem()
	boolMapperValueType = reflect.TypeOf((*BoolMapperValue)(nil)).
				Elem()
	jsonUnmarshalerType = reflect.TypeOf((*json.Unmarshaler)(nil)).
				Elem()
	textUnmarshalerType = reflect.TypeOf((*encoding.TextUnmarshaler)(nil)).
				Elem()
	binaryUnmarshalerType = reflect.TypeOf((*encoding.BinaryUnmarshaler)(nil)).
				Elem()
)
...
func (r *DecodeContext) WithScanner(
	scan *Scanner,
) *DecodeContext {
...
func (m *mapperValueAdapter) Decode(
	ctx *DecodeContext,
	target reflect.Value,
) error {
	if target.Type().Implements(mapperValueType) {
		return target.Interface().(MapperValue).Decode(
			ctx,
		) //nolint
	}
	return target.Addr().Interface().(MapperValue).Decode(
		ctx,
	) //nolint
}
...
func (m *textUnmarshalerAdapter) Decode(
	ctx *DecodeContext,
	target reflect.Value,
) error {
	var value string
	...
	if target.Type().
		...
		return target.Interface().(encoding.TextUnmarshaler).UnmarshalText(
			[]byte(value),
		) //nolint
	}
	return target.Addr().Interface().(encoding.TextUnmarshaler).UnmarshalText(
		[]byte(value),
	) //nolint
}
...
func (m *binaryUnmarshalerAdapter) Decode(
	ctx *DecodeContext,
	target reflect.Value,
) error {
	var value string
	...
	if target.Type().
		Implements(binaryUnmarshalerType) {
		return target.Interface().(encoding.BinaryUnmarshaler).UnmarshalBinary(
			[]byte(value),
		) //nolint
	}
	return target.Addr().Interface().(encoding.BinaryUnmarshaler).UnmarshalBinary(
		[]byte(value),
	) //nolint
}
...
func (j *jsonUnmarshalerAdapter) Decode(
	ctx *DecodeContext,
	target reflect.Value,
) error {
	var value string
	...
	if target.Type().
		Implements(jsonUnmarshalerType) {
		return target.Interface().(json.Unmarshaler).UnmarshalJSON(
			[]byte(value),
		) //nolint
	}
	return target.Addr().Interface().(json.Unmarshaler).UnmarshalJSON(
		[]byte(value),
	) //nolint
}
...
func intDecoder(
	...
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		if err != nil {
			return fmt.Errorf(
				"expected a valid %d bit int but got %q",
				bits,
				sv,
			)
		}
...
func uintDecoder(
	...
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		if err != nil {
			return fmt.Errorf(
				"expected a valid %d bit uint but got %q",
				bits,
				sv,
			)
		}
...
func mapDecoder(r *Registry) MapperFunc {
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		if ctx.Value.Flag != nil {
			...
			if t.IsEOL() {
				return fmt.Errorf(
					"missing value, expecting \"<key>=<value>%c...\"",
					mapsep,
				)
			}
			switch v := t.Value.(type) {
		...
		for !childScanner.Peek().IsEOL() {
			...
			if typ := ctx.Value.Tag.Type; typ != "" {
				...
				if len(parts) != 2 {
					return errors.New(
						"type:\"\" on map field must be in the form \"[<keytype>]:[<valuetype>]\"",
					)
				}
				keyTypeName, valueTypeName = parts[0], parts[1]
			}

			keyScanner := ScanAsType(
				FlagValueToken,
				key,
			)
			keyDecoder := r.ForNamedType(
				keyTypeName,
				el.Key(),
			)
			keyValue := reflect.New(el.Key()).
				Elem()
			...
			valueValue := reflect.New(el.Elem()).
				Elem()
...
func sliceDecoder(r *Registry) MapperFunc {
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		childDecoder := r.ForNamedType(
			ctx.Value.Tag.Type,
			el,
		)
		if childDecoder == nil {
			return fmt.Errorf(
				"no mapper for element type of %s",
				target.Type(),
			)
		}
		for !childScanner.Peek().IsEOL() {
			childValue := reflect.New(el).Elem()
			err := childDecoder.Decode(
				ctx.WithScanner(childScanner),
				childValue,
			)
			if err != nil {
				return err
...
func pathMapper(r *Registry) MapperFunc {
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		if target.Kind() != reflect.String {
			return fmt.Errorf(
				"\"path\" type must be applied to a string not %s",
				target.Type(),
			)
		}
		var path string
		err := ctx.Scan.PopValueInto(
			"file",
			&path,
		)
		if err != nil {
...
func existingFileMapper(r *Registry) MapperFunc {
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		if target.Kind() != reflect.String {
			return fmt.Errorf(
				"\"existingfile\" type must be applied to a string not %s",
				target.Type(),
			)
		}
		var path string
		...
		if !ctx.Value.Active ||
			(ctx.Value.Set && ctx.Value.Target.Type() == target.Type()) {
...
func existingDirMapper(r *Registry) MapperFunc {
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		if target.Kind() != reflect.String {
			return fmt.Errorf(
				"\"existingdir\" must be applied to a string not %s",
				target.Type(),
			)
		}
		var path string
	...
	return func(ctx *DecodeContext, target reflect.Value) error {
		if target.Kind() != reflect.Slice &&
			target.Elem().
				...
				reflect.Uint8 {
			return fmt.Errorf(
				"\"filecontent\" must be applied to []byte not %s",
				target.Type(),
			)
		}
		var path string
...
func (p ptrMapper) IsBoolFromValue(
	target reflect.Value,
) bool {
	...
	if bm, ok := nestedMapper.(BoolMapper); ok &&
		bm.IsBool() {
		return true
	}
	if bm, ok := nestedMapper.(BoolMapperExt); ok &&
		bm.IsBoolFromValue(target) {
		return true
	}
...
func (p ptrMapper) Decode(
	...
	if nestedMapper == nil {
		return fmt.Errorf(
			"cannot find mapper for %v",
			target.Type().Elem().String(),
		)
	}
...
func counterMapper() MapperFunc {
	return func(ctx *DecodeContext, target reflect.Value) error {
		...
		default:
			return fmt.Errorf(
				"type:\"counter\" must be used with a numeric field",
			)
		}
		return nil
	}
...
func (f *NamedFileContentFlag) Decode(
	ctx *DecodeContext,
) error { //nolint: revive
	var filename string
	err := ctx.Scan.PopValueInto(
