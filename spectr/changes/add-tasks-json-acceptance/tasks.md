...
- [x] 1.4 Implement `parseTasksMd()` function to parse tasks.md into structured format
- [x] 1.5 Implement `writeTasksJson()` function to write tasks.json with proper schema
- [x] 1.6 Add change validation step before conversion (reuse existing validation)
...
- [x] 2.4 Add `TaskStatus` type with `pending`, `in_progress`, `completed` values
...
- [x] 3.3 Ensure backward compatibility - fall back to tasks.md if no JSON exists
...
- [x] 4.4 Add auto-accept step to archive command when tasks.md still exists (with warning)

## 5. Slash Command Updates

- [x] 5.1 Update `.claude/commands/spectr/apply.md` to require `spectr accept` first
- [x] 5.2 Update `.gemini/commands/spectr/apply.toml` with accept requirement
- [x] 5.3 Update template files in `internal/initialize/templates/tools/slash-apply.md.tmpl`
