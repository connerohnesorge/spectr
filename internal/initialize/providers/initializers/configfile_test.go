...
func TestConfigFileInitializer_Init_CreateNew(t *testing.T) {
	...
	if !strings.Contains(updated, SpectrStartMarker) {
		t.Error("updated content should contain start marker")
	}

	if !strings.Contains(updated, SpectrEndMarker) {
		t.Error("updated content should contain end marker")
	}

	if !strings.Contains(updated, newContent) {
		t.Error("updated content should contain new content")
	}

	if !strings.Contains(updated, "existing content") {
		t.Error("updated content should preserve existing content")
	}
}

func TestConfigFileInitializer_UpdateExisting(t *testing.T) {
...
More content here`
	...
	if strings.Contains(updated, "old content") {
		t.Error("updated content should not contain old content")
	}
	...
	if !strings.Contains(updated, "# Configuration") {
		t.Error("updated content should preserve content before markers")
	}

	if !strings.Contains(updated, "More content here") {
		t.Error("updated content should preserve content after markers")
	}
}

func TestConfigFileInitializer_NoMarkersInExisting(t *testing.T) {
	...
	if !strings.Contains(updated, existingContent) {
		t.Error("updated content should preserve existing content")
	}
	...
	startIdx := strings.Index(updated, SpectrStartMarker)
	existingIdx := strings.Index(updated, existingContent)
	if startIdx < existingIdx {
		t.Error("markers should be appended after existing content")
	}
}
...
func TestConfigFileInitializer_IsSetup(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.wantSetup {
				t.Errorf("IsSetup() = %v, want %v", got, tt.wantSetup)
			}
		})
	}
}
...
func TestConfigFileInitializer_IsGlobal(t *testing.T) {
	...
	init.isGlobal = true
	if !init.IsGlobal() {
		t.Error("IsGlobal() should return true when set")
	}
}

func TestUpdateBetweenMarkers_Idempotent(t *testing.T) {
	...
	if wasUpdated2 {
		t.Error("second update should not report change (idempotent)")
	}

	if updated1 != updated2 {
		t.Error("content should be identical after idempotent update")
	}
}

func TestUpdateBetweenMarkers_PreservesStructure(t *testing.T) {
...
More content.`
	...
	if !strings.Contains(updated, "# Configuration File") {
		t.Error("should preserve title")
	}

	if !strings.Contains(updated, "Some preamble text.") {
		t.Error("should preserve preamble")
	}

	if !strings.Contains(updated, "Some epilogue text.") {
		t.Error("should preserve epilogue")
	}

	if !strings.Contains(updated, "## Another Section") {
	...
	if strings.Contains(updated, "old spectr content") {
