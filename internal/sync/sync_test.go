	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got) != len(tt.want) {
				t.Errorf(
					"buildStatusMap() returned %d items, want %d",
					len(got),
					len(tt.want),
				)
...
func TestUpdateTaskLine(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotLine, gotChange := updateTaskLine(
				tt.line,
				tt.statusMap,
			)
			if gotLine != tt.wantLine {
				t.Errorf(
					"updateTaskLine() line = %q, want %q",
					gotLine,
					tt.wantLine,
				)
			}
			if gotChange != tt.wantChange {
				t.Errorf(
					"updateTaskLine() changed = %v, want %v",
					gotChange,
					tt.wantChange,
				)
			}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Run sync
			syncCount, err := SyncTasksToMarkdown(
				tmpDir,
			)
			if err != nil {
				t.Fatalf(
					"SyncTasksToMarkdown() error = %v",
					err,
				)
			}
...
func TestSyncAllActiveChanges(t *testing.T) {
	...
	change1Dir := filepath.Join(
		changesDir,
		"change-one",
	)
	change2Dir := filepath.Join(
		changesDir,
		"change-two",
	)
	...
	if err != nil {
		t.Fatalf(
			"SyncAllActiveChanges() error = %v",
			err,
		)
	}

	// Verify change-one was updated
	gotMd1, _ := os.ReadFile(
		filepath.Join(change1Dir, "tasks.md"),
	)
	wantMd1 := "- [x] 1.1 Task\n"
	if string(gotMd1) != wantMd1 {
		t.Errorf(
			"change-one/tasks.md = %q, want %q",
			string(gotMd1),
			wantMd1,
		)
	}
	...
	gotMd2, _ := os.ReadFile(
		filepath.Join(change2Dir, "tasks.md"),
	)
	if string(gotMd2) != md2 {
		t.Errorf(
			"change-two/tasks.md = %q, want %q",
			string(gotMd2),
			md2,
		)
	}
	...
	gotMd3, _ := os.ReadFile(
		filepath.Join(archiveDir, "tasks.md"),
	)
