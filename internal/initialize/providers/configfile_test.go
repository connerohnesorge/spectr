...
func TestConfigFileInitializer_Init_NewFile(
	...
	if len(result.CreatedFiles) != 1 ||
		result.CreatedFiles[0] != testClaudeMd {
		t.Errorf(
			"CreatedFiles = %v, want [CLAUDE.md]",
			result.CreatedFiles,
		)
	}
...
func TestConfigFileInitializer_Init_UpdateBetweenMarkers(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
	...
	if len(result.UpdatedFiles) != 1 ||
		result.UpdatedFiles[0] != testClaudeMd {
		t.Errorf(
			"UpdatedFiles = %v, want [CLAUDE.md]",
			result.UpdatedFiles,
		)
	}
...
func TestConfigFileInitializer_Init_CaseInsensitiveMarkers(
	t *testing.T,
) {
	tests := []struct {
		name     string
...
func TestConfigFileInitializer_Init_OrphanedStartWithTrailingEnd(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
...
func TestConfigFileInitializer_Init_OrphanedStartWithNoEnd(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
...
func TestConfigFileInitializer_Init_NoMarkers(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
...
func TestConfigFileInitializer_Init_TemplateContextUsage(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
...
func TestConfigFileInitializer_Init_ErrorOrphanedEndMarker(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
	...
	if err == nil {
		t.Fatal(
			"Init() should fail with orphaned end marker",
		)
	}
	...
	if !strings.Contains(
		...
		t.Errorf(
			"error message = %q, want to contain 'orphaned end marker'",
			err.Error(),
		)
	}
}

func TestConfigFileInitializer_Init_ErrorNestedStartMarkers(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
	...
	if err == nil {
		t.Fatal(
			"Init() should fail with nested start markers",
		)
	}
	...
	if !strings.Contains(
		...
		t.Errorf(
			"error message = %q, want to contain 'nested start marker'",
			err.Error(),
		)
	}
}

func TestConfigFileInitializer_Init_ErrorMultipleStartMarkers(
	t *testing.T,
) {
	// Setup
	projectFs := afero.NewMemMapFs()
	...
	if err == nil {
		t.Fatal(
			"Init() should fail with multiple start markers",
		)
	}
	...
	if !strings.Contains(
		...
		t.Errorf(
			"error message = %q, want to contain 'multiple start markers'",
			err.Error(),
		)
	}
}
...
func TestConfigFileInitializer_dedupeKey(
	t *testing.T,
) {
	tests := []struct {
		name string
