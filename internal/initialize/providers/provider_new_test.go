...
// TestProviderV2Interface verifies that ProviderV2 interface works correctly.
func TestProviderV2Interface(t *testing.T) {
	t.Run("ProviderV2Func implements ProviderV2", func(t *testing.T) {
		var p ProviderV2 = ProviderV2Func(func(ctx context.Context) []Initializer {
			return nil
		})
		if p == nil {
			t.Error("ProviderV2Func should implement ProviderV2 interface")
		}
	})

	t.Run("ProviderV2Func returns initializers", func(t *testing.T) {
		expected := []Initializer{
			NewDirectoryInitializer("test"),
		}
		p := ProviderV2Func(func(ctx context.Context) []Initializer {
			return expected
		})
...
// TestAllProvidersImplementProviderV2 verifies all registered providers implement ProviderV2.
func TestAllProvidersImplementProviderV2(t *testing.T) {
	...
	for _, reg := range allRegs {
		t.Run(reg.ID, func(t *testing.T) {
			// Check that Provider implements ProviderV2
			if reg.Provider == nil {
	...
	t.Run("providers are sorted by priority", func(t *testing.T) {
		for i := 1; i < len(allRegs); i++ {
			if allRegs[i-1].Priority > allRegs[i].Priority {
				t.Errorf("Providers not sorted by priority: %q (priority %d) comes before %q (priority %d)",
					allRegs[i-1].ID, allRegs[i-1].Priority,
					allRegs[i].ID, allRegs[i].Priority)
			}
		}
	})
...
func TestProviderCount(t *testing.T) {
	...
	if count != expectedCount {
		t.Errorf("CountV2() = %d, want %d", count, expectedCount)
		t.Log("Registered providers:")
		for _, reg := range AllV2() {
	...
	tests := []struct {
		...
		},
		{
			...
			reg: Registration{
				ID:       "",
				Name:     "Test Provider",
				Provider: ProviderV2Func(func(ctx context.Context) []Initializer { return nil }),
			},
			wantErr: ErrEmptyID,
		},
		{
...
func TestInitializerIsGlobal(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			init:       NewGlobalSlashCommandsInitializer(".config/commands", ".md", FormatMarkdown),
			wantGlobal: true,
		},
	}
