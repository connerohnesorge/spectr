	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.HasPrefix(result, "---") {
				t.Error(
					"markdown command should start with frontmatter",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"markdown command should contain description field",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"markdown command should contain start marker",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"markdown command should contain end marker",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"markdown command should contain provided content",
				)
			}

			// Verify structure: frontmatter, then markers with content
			frontmatterEnd := strings.Index(
				result[3:],
				"---",
			)
			if frontmatterEnd == -1 {
				t.Error(
					"markdown command should have closing frontmatter",
				)
			}

			startMarkerIdx := strings.Index(
				result,
				SpectrStartMarker,
			)
			if startMarkerIdx < frontmatterEnd {
				t.Error(
					"start marker should come after frontmatter",
				)
			}
			...
			if contentIdx < startMarkerIdx {
				t.Error(
					"content should come after start marker",
				)
			}

			endMarkerIdx := strings.Index(
				result,
				SpectrEndMarker,
			)
			if endMarkerIdx < contentIdx {
				t.Error(
					"end marker should come after content",
				)
			}
		})
	}
...
func TestFormatTOMLCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.HasPrefix(result, "#") {
				t.Error(
					"TOML command should start with comment",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"TOML command should contain [[agent]] section",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"TOML command should contain start marker",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"TOML command should contain end marker",
				)
			}
			...
			if !strings.Contains(
				...
				t.Error(
					"TOML command should contain provided content",
				)
			}

			// Verify structure: comment, [[agent]], markers with content
			agentIdx := strings.Index(
				result,
				"[[agent]]",
			)
			if agentIdx == -1 {
				t.Error(
					"TOML command should have [[agent]] section",
				)
			}
			...
			if startMarkerIdx < agentIdx {
				t.Error(
					"start marker should come after [[agent]]",
				)
			}
...
func TestSlashCommandsInitializer_MultipleCommands(
	t *testing.T,
) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if init.IsSetup(fs, cfg) {
				t.Error(
					"IsSetup() should return false before files are created",
				)
			}
			...
			if !init.IsSetup(fs, cfg) {
				t.Error(
					"IsSetup() should return true after all files are created",
				)
			}
		})
	}
...
func TestSlashCommandsInitializer_UnsupportedExtension(
	t *testing.T,
) {
...
func TestUpdateBetweenMarkers_WithSlashCommands(
	t *testing.T,
) {
