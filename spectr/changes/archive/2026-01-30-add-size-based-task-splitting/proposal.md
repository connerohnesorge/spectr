...
Claude Code and other AI agents work best when they can read entire task files
in one operation. By automatically splitting large `tasks.md` files into
multiple smaller `tasks.jsonc` files at accept time, we ensure optimal agent
readability while maintaining human-friendly single-file authoring.
...
- **ADDED**: Root `tasks.jsonc` with references to split files
  (`tasks-1.jsonc`, `tasks-2.jsonc`, etc.)
- **ADDED**: Child task file format with full headers documenting origin
  (parent change ID, parent task ID)
- **ADDED**: Task reference syntax using `"children": "$ref:tasks-N.jsonc"`
...
- **MODIFIED**: `spectr accept` command to implement splitting logic with
  100-line threshold
- **MODIFIED**: Task ID schema to support hierarchical IDs (e.g., `5.1`,
  `5.1.1`)
...
- **Affected code**:
  ...
  - `internal/parsers/parsers.go` - add ReadTasksFile function for
