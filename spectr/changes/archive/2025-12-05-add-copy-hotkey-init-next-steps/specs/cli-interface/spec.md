...
- **AND** no success message is displayed (silent exit, consistent with list
  mode Enter behavior)
...
- **WHEN** the Next Steps completion screen is displayed after successful
  initialization
...
- **WHEN** the user presses 'c' in an SSH/remote session without native
  clipboard access
