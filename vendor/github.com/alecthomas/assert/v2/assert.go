...
func HasPrefix(
	...
	t.Helper()
	msg := formatMsgAndArgs(
		"Expected string to have prefix:",
		msgAndArgs...)
	t.Fatalf(
		"%s\nPrefix: %q\nString: %q\n",
		msg,
...
func HasSuffix(
	...
	t.Helper()
	msg := formatMsgAndArgs(
		"Expected string to have suffix:",
		msgAndArgs...)
	t.Fatalf(
		"%s\nSuffix: %q\nString: %q\n",
		msg,
...
func Equal[T any](
	t testing.TB,
	expected, actual T,
	msgArgsAndCompareOptions ...any,
) {
	msgArgsAndCompareOptions, compareOptions := extractCompareOptions(
		msgArgsAndCompareOptions...)
	if objectsAreEqual(
		expected,
		actual,
		compareOptions...) {
	...
	t.Fatalf(
		"%s\n%s",
		msg,
		Diff(expected, actual, compareOptions...),
	)
}
...
func NotEqual[T any](
	...
	t.Helper()
	msg := formatMsgAndArgs(
		"Expected values to not be equal but both were:",
		msgArgsAndCompareOptions...)
	t.Fatalf(
		"%s\n%s",
		msg,
...
func NotContains(
	...
	msg := formatMsgAndArgs(
		...
		msgAndArgs...)
	quotedHaystack, quotedNeedle, positions := needlePosition(
		haystack,
		needle,
	)
	t.Fatalf(
...
func SliceContains[T any](
	...
	msg := formatMsgAndArgs(
		"Haystack does not contain needle.",
		msgAndArgs...)
	needleRepr := repr.String(
		needle,
		repr.Indent("  "),
	)
	haystackRepr := repr.String(
		haystack,
		repr.Indent("  "),
	)
	t.Fatalf(
...
func Zero[T any](
	...
	if (val.Kind() == reflect.Slice || val.Kind() == reflect.Map || val.Kind() == reflect.Array) &&
		val.Len() == 0 {
		return
	}
	t.Helper()
	msg := formatMsgAndArgs(
		"Expected a zero value but got:",
		msgAndArgs...)
	t.Fatalf(
		"%s\n%s",
		msg,
...
func IsError(
	...
	t.Fatal(
		formatMsgAndArgs(
			fmt.Sprintf(
				"Error tree %+v should contain error %q",
				err,
				target,
			),
			msgAndArgs...),
...
func NotIsError(
	...
	t.Fatal(
		formatMsgAndArgs(
			fmt.Sprintf(
				"Error tree %+v should NOT contain error %q",
				err,
				target,
			),
			msgAndArgs...),
...
func formatMsgAndArgs(
	...
	if !ok {
		panic(
			"message argument to assert function must be a fmt string",
		)
	}
