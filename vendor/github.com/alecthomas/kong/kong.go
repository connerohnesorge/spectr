...
var callbackReturnSignature = reflect.TypeOf((*error)(nil)).
	Elem()
...
func New(
	...
	for _, embed := range k.embedded {
		tag, err := parseTagString(
			strings.Join(embed.tags, " "),
		)
		if err != nil {
			return nil, err
		}
		...
		for _, child := range node.Children {
			...
			k.Model.Children = append(
				k.Model.Children,
				child,
			)
		}
		k.Model.Flags = append(
			k.Model.Flags,
			node.Flags...)
	}
	...
	for _, dcmd := range k.dynamicCommands {
		tag, terr := parseTagString(
			strings.Join(dcmd.tags, " "),
		)
		if terr != nil {
			return nil, terr
		}
	...
	for xor, xorSet := range xorGroups {
		for and, andSet := range andGroups {
			...
			if len(overlappingEntries) > 1 {
				return fmt.Errorf(
					"invalid xor and combination, %s and %s overlap with more than one: %s",
					xor,
					and,
					overlappingEntries,
				)
...
func (k *Kong) interpolateValue(
	...
	if value.Flag != nil {
		...
		value.Flag.PlaceHolder, err = interpolate(
		...
		if err != nil {
			return fmt.Errorf(
				"placeholder value for %s: %s",
				value.Summary(),
				err,
			)
		}
	}
	value.Help, err = interpolate(
		value.Help,
		vars,
		updatedVars,
	)
...
func (k *Kong) applyHook(
	...
	return k.applyHookToDefaultFlags(
		ctx,
		ctx.Path[0].Node(),
		name,
	)
...
func (k *Kong) applyHookToDefaultFlags(
	...
	return Visit(
		node,
		func(n Visitable, next Next) error {
			node, ok := n.(*Node)
			if !ok {
				return next(nil)
			}
			binds := k.bindings.clone().
				add(ctx).
				add(node.Vars().CloneWith(k.vars))
			for _, flag := range node.Flags {
				if !flag.HasDefault ||
					ctx.values[flag.Value].IsValid() ||
					!flag.Target.IsValid() {
					continue
				}
