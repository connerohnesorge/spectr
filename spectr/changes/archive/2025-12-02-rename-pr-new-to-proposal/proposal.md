...
Rename the `spectr pr new` subcommand to `spectr pr proposal` to align with the slash command naming convention (`/spectr:proposal`). This creates a consistent vocabulary across the CLI and IDE integrations.
...
- **CLI breaking change**: Users must update scripts/documentation using `spectr pr new`
- **Internal refactor**: Rename `ModeNew` → `ModeProposal`, `PRNewCmd` → `PRProposalCmd`
...
3. **Rename slash command to `/spectr:new`**: "proposal" better describes the action
