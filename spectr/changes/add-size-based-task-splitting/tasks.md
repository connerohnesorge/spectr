...
**Issue**: `json.MarshalIndent()` double-escapes quotes in task descriptions, producing invalid JSONC. This breaks file generation and agent readability.

- [ ] 0.1 Create new file `cmd/jsonc.go` for custom JSONC marshalling
- [ ] 0.2 Implement `escapeJSONString()` function to properly escape quotes without double-escaping
- [ ] 0.3 Implement `MarshalTaskToJSONLines()` to convert single Task to JSONC format
- [ ] 0.4 Implement `MarshalTasksToJSONLines()` to convert task array with proper indentation
- [ ] 0.5 Implement `ValidateJSONCSyntax()` to validate output is valid JSONC before writing
...
- [ ] 0.9 Update `cmd/accept_writer.go` line 77-81 to use custom marshaller instead of `json.MarshalIndent()`
- [ ] 0.10 Update `writeTasksJSONC()` to validate JSONC output before writing
- [ ] 0.11 Run existing tests to verify no regressions

## 1. Schema Updates

- [x] 1.1 Update `internal/parsers/types.go` to add `Children` field to Task struct with JSON tag `children,omitempty`
...
- [x] 2.1 Add `countLines()` function in `cmd/accept.go` to count lines in tasks.md file
...
- [x] 3.1 Add `Section` struct to represent a section with name, tasks, and line range
- [x] 3.2 Add `parseSections()` function to extract sections from tasks.md by `## N.` headers
...
- [x] 4.1 Add `shouldSplitSection()` function to check if a section exceeds 100 lines
- [x] 4.2 Add `parseSubsections()` function to group tasks by ID prefix (e.g., "1.1", "1.2")
...
- [x] 5.2 Add logic to preserve existing IDs from tasks.md and add parent prefix for children
...
- [x] 9.1 Add `writeHierarchicalTasks()` orchestration function in `cmd/accept_writer.go`
...
- [x] 9.4 Add `deleteOldChildFiles()` function to clean up tasks-*.jsonc before regeneration
- [x] 9.5 Update existing `writeTasksJSONC()` to route to hierarchical path when needed
