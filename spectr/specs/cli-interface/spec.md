...
The accept command SHALL generate `tasks.jsonc` files conforming to a
versioned schema with structured task objects, header comments, and optional
hierarchical structure for large changes.
...
- AND `version` SHALL be integer 2 for this schema version (backwards
...
- AND the header SHALL indicate the file is machine-generated by
  `spectr accept`
...
- AND the header SHALL document valid status transitions: pending ->
  in_progress -> completed
- AND the header SHALL explain that agents should mark a task
...
- THEN it SHALL have `id` field containing the task identifier (e.g.,
  "1.1")
...
- AND it SHALL have `status` field with value "pending", "in_progress", or
  "completed"
- AND it MAY have `children` field with value "$ref:`<path>`" for
  hierarchical tasks
