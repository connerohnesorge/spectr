...
func TestInstanceRegistry(t *testing.T) {
	...
	// Register a provider (using the global registry to get a valid provider)
	claudeProvider := Get("claude-code")
	if claudeProvider == nil {
		t.Fatal("Claude provider not found in global registry")
	}
