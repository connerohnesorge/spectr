...
The current VHS tape files are cluttered with verbose `cat > file << 'EOF'`
heredoc blocks that obscure the actual spectr commands being demonstrated. This
makes it hard for users to understand what the tool does at a glance.
Additionally, there's no easy way to regenerate all demo GIFs at once.
...
- Add a `generate-gif` command (via flake.nix) to build all VHS GIFs in one
  command
- Keep demos focused on showcasing spectr commands, not file creation
  boilerplate
