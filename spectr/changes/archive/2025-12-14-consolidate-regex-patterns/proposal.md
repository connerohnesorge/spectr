...
The codebase has 28+ regex patterns scattered across 6 files for parsing
markdown structure:
...
1. **Duplication**: Same patterns (e.g., `^###\s+Requirement:`) appear in 4+
  files
...
5. **Migration friction**: The upcoming `replace-regex-with-blackfriday` change
  must touch all 6 files
...
- **NEW**: `internal/regex/` package with pre-compiled patterns and matching
  functions
...
|----------|--------|-----------|
| Package name | `internal/regex/` | Clear; not `markdown` (avoids confusion) |
| Pattern compile | Package `var`+`MustCompile` | Single init compile; panic OK |
| Function sigs | `(matches, found bool)` | Consistent API; no nil ambiguity |
| Section extract | Keep in callers | Only patterns consolidated |
| Utility patterns | Leave inline | Simple & context-specific |
...
This change is a **prerequisite** for `replace-regex-with-blackfriday`. After
this change:
