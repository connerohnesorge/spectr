...
func (s *Screen) costEffective(
	...
	if blank {
		...
		costBeforeMove = s.updateCostBlank(
			s.newbuf.Line(to),
		)
	} else {
	...
	costBeforeMove += s.updateCost(
		s.curbuf.Line(newFrom),
		s.newbuf.Line(from),
	)
	...
	if newFrom == from {
		...
		costAfterMove = s.updateCostBlank(
			s.newbuf.Line(from),
		)
	} else {
