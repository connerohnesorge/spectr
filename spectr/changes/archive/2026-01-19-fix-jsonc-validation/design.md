...
```text
tasks.md → parseTasksMd() → []Task → json.MarshalIndent() → JSONC string → Write
  to disk
```
...
1. **Performance**: Should we validate on every write or only in tests?
   - **Decision**: Validate on every write (fail fast), but make it optional via
     flag for performance-critical paths
