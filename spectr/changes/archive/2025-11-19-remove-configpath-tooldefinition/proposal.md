...
The `ConfigPath` field in `ToolDefinition` (internal/init/models.go:24) is
misleading and serves no functional purpose. While it's set for each tool in the
registry and returned by `getToolFileInfo()`, it's never actually used to
determine where files are created. Each configurator hardcodes its own file path
independently:
...
The values stored in `ConfigPath` (like `.claude/claude.json`,
`.cline/cline_mcp_settings.json`) don't match the actual files created and are
never referenced when creating configuration files. This creates confusion and
...
- Update `getToolFileInfo()` to return actual file paths from configurators
  instead of using ConfigPath
- Update tests that reference ConfigPath
- No user-facing behavior changes (files are still created in the same
  locations)
