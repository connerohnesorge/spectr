...
func TestItem_ID_EmptyWhenNil(t *testing.T) {
	...
	if got := item.ID(); got != "" {
		t.Errorf(
			"Item.ID() with nil Change = %v, want empty string",
			got,
		)
	}
	...
	if got := item.ID(); got != "" {
		t.Errorf(
			"Item.ID() with nil Spec = %v, want empty string",
			got,
		)
	}
}
...
func TestItem_Title_EmptyWhenNil(t *testing.T) {
	...
	if got := item.Title(); got != "" {
		t.Errorf(
			"Item.Title() with nil Change = %v, want empty string",
			got,
		)
	}
	...
	if got := item.Title(); got != "" {
		t.Errorf(
			"Item.Title() with nil Spec = %v, want empty string",
			got,
		)
	}
}
...
func TestItemList_FilterByType(t *testing.T) {
	...
	spec1 := NewSpecItem(
		SpecInfo{ID: "spec1", Title: "Spec 1"},
	)
	spec2 := NewSpecItem(
		SpecInfo{ID: "spec2", Title: "Spec 2"},
	)
	...
	if len(changes) != 2 {
		t.Errorf(
			"FilterByType(ItemTypeChange) length = %v, want 2",
			len(changes),
		)
	}
	...
	if len(specs) != 2 {
		t.Errorf(
			"FilterByType(ItemTypeSpec) length = %v, want 2",
			len(specs),
		)
	}
}
...
func TestItemList_Changes(t *testing.T) {
	...
	if changes[0].ID != "change1" {
		t.Errorf(
			"Changes()[0].ID = %v, want change1",
			changes[0].ID,
		)
	}
	if changes[1].ID != "change2" {
		t.Errorf(
			"Changes()[1].ID = %v, want change2",
			changes[1].ID,
		)
	}
}
...
func TestItemList_Specs(t *testing.T) {
	...
	if specs[0].ID != "spec1" {
		t.Errorf(
			"Specs()[0].ID = %v, want spec1",
			specs[0].ID,
		)
	}
	if specs[1].ID != "spec2" {
		t.Errorf(
			"Specs()[1].ID = %v, want spec2",
			specs[1].ID,
		)
	}
}
...
func TestItemList_EmptyLists(t *testing.T) {
	...
	if len(changes) != 0 {
		t.Errorf(
			"Empty ItemList.Changes() length = %v, want 0",
			len(changes),
		)
	}
	...
	if len(specs) != 0 {
		t.Errorf(
			"Empty ItemList.Specs() length = %v, want 0",
			len(specs),
		)
	}
	...
	if len(filtered) != 0 {
		t.Errorf(
			"Empty ItemList.FilterByType() length = %v, want 0",
			len(filtered),
		)
	}
}
