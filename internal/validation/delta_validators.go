...
func validateAddedRequirements(
	...
	for _, req := range requirements {
		normalized := NormalizeRequirementName(
			req.Name,
		)
		reqPath := fmt.Sprintf(
		...
		if len(req.Scenarios) == 0 &&
			...
			malformedLine := findMalformedScenarioLineInDelta(
				lines,
				reqLine,
			)
			issues = append(
...
func validateRenamedRequirements(
	...
	var issues []ValidationIssue
	renames := parseRenamedRequirements(
		renamedContent,
	)
	...
	for _, rename := range renames {
		if rename.FromName == "" ||
			...
			malformedLine := findMalformedRenamedLine(
				lines,
				sectionLine,
			)
			issues = append(
		...
		normalizedFrom := NormalizeRequirementName(
			rename.FromName,
		)
		normalizedTo := NormalizeRequirementName(
			rename.ToName,
		)
		reqPath := fmt.Sprintf(
		...
		reqFromLine := findRenamedPairLine(
			lines,
			rename.FromName,
			sectionLine,
		)
		reqToLine := findRenamedPairLine(
			lines,
			rename.ToName,
			reqFromLine,
		)
