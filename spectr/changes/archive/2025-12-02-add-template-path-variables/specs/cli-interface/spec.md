...
- **THEN** the path SHALL be expressed using Go template syntax (e.g., `{{
  .BaseDir }}`, `{{ .SpecsDir }}`)
- **AND** hardcoded `spectr/` strings SHALL NOT appear in template files for
  path references
- **AND** the rendered output SHALL contain the actual path values from the
  context

#### Scenario: Default context produces backward-compatible output

- **WHEN** `DefaultTemplateContext()` is used for rendering
- **THEN** the rendered output SHALL be identical to the previous hardcoded
  output
...
- **WHEN** `RenderAgents()`, `RenderInstructionPointer()`, or
  `RenderSlashCommand()` is called
