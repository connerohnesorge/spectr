...
The system SHALL count tasks in `tasks.md` files by identifying lines matching the pattern `- [ ]` or `- [x]` (case-insensitive), with completed tasks marked by `[x]`. The system SHALL prefer `tasks.jsonc` when present, falling back to `tasks.json` for backward compatibility.
...
#### Scenario: Count tasks from legacy JSON
- WHEN the system counts tasks and `tasks.jsonc` does not exist but `tasks.json` exists
...
- WHEN the system cannot find or read a `tasks.md`, `tasks.jsonc`, or `tasks.json` file for a change
- THEN it reports `taskStatus` as `{ total: 0, completed: 0 }`
- AND continues processing without error

#### Scenario: JSONC takes precedence over JSON and Markdown
- WHEN `tasks.jsonc`, `tasks.json`, and `tasks.md` exist
- THEN the system reads from `tasks.jsonc`
- AND ignores `tasks.json` and `tasks.md`
...
- WHEN the accept command creates tasks.jsonc
- THEN the header SHALL document the three valid status values: "pending", "in_progress", "completed"
- AND the header SHALL indicate the file is machine-generated by `spectr accept`
- AND the header SHALL use `//` line comment syntax
