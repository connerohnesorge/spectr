...
func NewWizardModel(
	...
	// Create filesystem and config for checking if providers are already configured
	projectFs := afero.NewBasePathFs(
		afero.NewOsFs(),
		projectPath,
	)
	cfg := providers.NewDefaultConfig()
	ctx := context.Background()

	// Check each provider to see if it's already configured
	for _, reg := range allProviders {
		...
		// Pre-select providers that are already configured
		if isConfigured {
...
func (m WizardModel) handleSelectKeys(
	...
	case " ":
		...
		if m.cursor < len(m.filteredProviders) {
			reg := m.filteredProviders[m.cursor]
	...
	case "a":
		...
		for _, reg := range m.allProviders {
	...
	if query == "" {
		...
		for _, reg := range m.allProviders {
			if strings.Contains(strings.ToLower(reg.Name), query) {
				m.filteredProviders = append(m.filteredProviders, reg)
			}
		}
	}
...
// isProviderConfigured checks if a provider is already configured by checking
// if all its initializers report IsSetup() as true.
func isProviderConfigured(
	...
	// Get the provider's initializers
	initializers := reg.Provider.Initializers(ctx)

	// If there are no initializers, consider it not configured
	if len(initializers) == 0 {
		return false
	}

	// Check if ALL initializers are already set up
	for _, init := range initializers {
		// Skip global initializers (they use a different filesystem)
		if init.IsGlobal() {
			continue
		}

		// If any initializer is not set up, the provider is not configured
