...
- Search existing work: Read `spectr/changes/` and `spectr/specs/` directories
  (use `rg` for full-text search)
...
1. Review `spectr/project.md` and read `spectr/specs/` and `spectr/changes/`
   directories to understand current context.
...
6. **For each task in `tasks.jsonc`:**
   ...
   - Do NOT batch status updates - update each task individually as you
     complete it
...
2. Identify drift: new features (ADDED), changed behavior (MODIFIED), removed
   features (REMOVED)
...
- Enumerate specs: Read `spectr/specs/` directory (or `spectr spec list --long`
...
spectr accept <change-id>    # Accept proposal, convert tasks.md to tasks.jsonc
...
- Tasks: Read `spectr/changes/<change-id>/tasks.jsonc` (preferred) or
  `tasks.md` directly
...
- MODIFIED: Changes the behavior, scope, or acceptance criteria of an existing
  requirement. Always paste the full, updated requirement content (header +
  all scenarios). The sync process will replace the entire requirement with
  what you provide here; partial deltas will drop previous details.
...
previous text. This causes loss of detail when syncing. If you aren't
