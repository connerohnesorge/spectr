...
Automates Spectr validation in GitHub Actions to ensure specification compliance
for all commits and pull requests.
...
- **WHEN** a developer pushes changes to any `.tape` file in
  `docs/src/assets/vhs/`
- **THEN** the VHS workflow executes automatically
- **AND** VHS processes all tape files to generate updated GIFs
- **AND** the generated GIFs are committed back to the `docs/src/assets/gifs/`
  directory
...
- **AND** the commit SHALL only include `*.gif` files from
  `docs/src/assets/gifs/`
