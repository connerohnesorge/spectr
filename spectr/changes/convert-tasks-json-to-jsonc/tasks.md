...
- [ ] 1.3 Update `CountTasks` to check `tasks.jsonc` first, then fall back to `tasks.json`, then `tasks.md`

## 2. Accept Command Changes

- [ ] 2.1 Add `tasksJSONHeader` constant with usage instructions in `cmd/accept.go`
...
- [ ] 3.3 Add `TestCountTasks_JsoncPreferredOverJson` for fallback behavior
- [ ] 3.4 Update `cmd/accept_test.go` references from `tasks.json` to `tasks.jsonc`
