...
func TestViewCmd_Integration_TextOutput(
	t *testing.T,
) {
	...
	if err != nil {
		t.Fatalf(
			"Failed to change to project root: %v",
			err,
		)
	}
	...
	for _, expected := range expectedElements {
		if !strings.Contains(output, expected) {
			t.Errorf(
				"Expected text output to contain %q, but it didn't.\nFull output:\n%s",
				expected,
				output,
			)
		}
	...
	t.Logf(
		"Text output successfully generated (%d bytes)",
		len(output),
	)
}

// TestViewCmd_Integration_JSONOutput tests the view command with JSON output
func TestViewCmd_Integration_JSONOutput(
	t *testing.T,
) {
	// Save original working directory
	originalWd, err := os.Getwd()
	...
	if err != nil {
		t.Fatalf(
			"ViewCmd.Run() with JSON failed: %v",
			err,
		)
	}
	...
	if data.ActiveChanges == nil {
		t.Error(
			"Expected ActiveChanges to be initialized (not null)",
		)
	}
	if data.CompletedChanges == nil {
		t.Error(
			"Expected CompletedChanges to be initialized (not null)",
		)
	}
	if data.Specs == nil {
		t.Error(
			"Expected Specs to be initialized (not null)",
		)
	}

	t.Log(
		"JSON output successfully generated and parsed:",
	)
	t.Logf(
...
func TestViewCmd_Integration_NOCOLOREnvironment(
	t *testing.T,
) {
	...
	if err != nil {
		t.Fatalf(
			"ViewCmd.Run() with NO_COLOR failed: %v",
			err,
		)
	}
	...
	if !strings.Contains(
		...
		t.Error(
			"Expected dashboard title even with NO_COLOR",
		)
	}
	...
	t.Logf(
		"NO_COLOR environment variable respected (output: %d bytes)",
		len(output),
	)
}

// TestViewCmd_Integration_MissingSpectrDirectory tests behavior when spectr dir is missing
func TestViewCmd_Integration_MissingSpectrDirectory(
	t *testing.T,
) {
	...
	if err != nil {
		t.Fatalf(
			"ViewCmd.Run() unexpectedly failed: %v",
			err,
		)
	}
	...
	if !strings.Contains(
		...
		t.Error(
			"Expected output to show 0 specs and requirements",
		)
	}
	if !strings.Contains(
		output,
		...
		t.Error(
			"Expected output to show 0 active changes",
		)
	}
	if !strings.Contains(
		output,
	...
	t.Log(
		"Correctly handled missing spectr directory with empty dashboard",
	)
}
