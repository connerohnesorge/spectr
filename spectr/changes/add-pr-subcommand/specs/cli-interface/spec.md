...
The system SHALL provide a `spectr pr` command with `archive` and `new` subcommands for creating pull requests from Spectr changes using git worktree isolation.

#### Scenario: User runs spectr pr without subcommand
- **WHEN** user runs `spectr pr` without a subcommand
- **THEN** help text is displayed showing available subcommands (archive, new)
...
#### Scenario: User runs spectr pr new
- **WHEN** user runs `spectr pr new <change-id>`
- **THEN** the system executes the new PR workflow
...
### Requirement: PR New Subcommand
The `spectr pr new` subcommand SHALL create a pull request containing a Spectr change proposal for review, copying the change to an isolated git worktree without archiving.

#### Scenario: New PR workflow execution
- **WHEN** user runs `spectr pr new <change-id>`
- **THEN** the system creates a temporary git worktree on branch `spectr/<change-id>`
...
#### Scenario: New PR does not archive
- **WHEN** user runs `spectr pr new <change-id>`
...
#### Scenario: New PR validates change first
- **WHEN** user runs `spectr pr new <change-id>`
...
Both `spectr pr archive` and `spectr pr new` subcommands SHALL support common flags for controlling PR creation behavior.
...
#### Scenario: New commit message format
- **WHEN** `spectr pr new` commits changes
- **THEN** the commit message includes:
  - Title: `spectr(proposal): <change-id>`
  - Proposal location path
  - Attribution: "Generated by: spectr pr new"
...
#### Scenario: New PR body content
- **WHEN** PR is created for new proposal
- **THEN** the PR body includes:
...
- **WHEN** user runs `spectr pr new refactor`
