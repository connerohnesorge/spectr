...
var (
	...
	procReadConsoleInputW = modkernel32.NewProc(
		"ReadConsoleInputW",
	)
	procPeekConsoleInputW = modkernel32.NewProc(
		"PeekConsoleInputW",
	)
	procGetNumberOfConsoleInputEvents = modkernel32.NewProc(
...
// ReadNConsoleInputs.
func WinReadConsoleInput(
	consoleInput windows.Handle,
	buffer *InputRecord,
	length uint32,
	numberOfEventsRead *uint32,
) error {
	r, _, e := syscall.Syscall6(
		...
		uintptr(length),
		uintptr(
			unsafe.Pointer(numberOfEventsRead),
		),
		0,
		0,
	)
...
func ReadNConsoleInputs(
	...
	if maxEvents == 0 {
		return nil, fmt.Errorf(
			"maxEvents cannot be zero",
		)
	}

	inputRecords := make([]InputRecord, maxEvents)
	n, err := ReadConsoleInput(
		console,
		inputRecords,
	)
...
func ReadConsoleInput(
	...
	if len(inputRecords) == 0 {
		return 0, fmt.Errorf(
			"size of input record buffer cannot be zero",
		)
	}

	var read uint32
	err := WinReadConsoleInput(
		console,
		&inputRecords[0],
		uint32(len(inputRecords)),
		&read,
	)
...
func WinGetNumberOfConsoleInputEvents(
	consoleInput windows.Handle,
	numberOfEvents *uint32,
) error {
	r, _, e := syscall.Syscall6(
		procGetNumberOfConsoleInputEvents.Addr(),
		2,
		uintptr(
			consoleInput,
		),
...
func GetNumberOfConsoleInputEvents(
	...
	var nEvents uint32
	err := WinGetNumberOfConsoleInputEvents(
		console,
		&nEvents,
	)
